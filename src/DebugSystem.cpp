...
void DebugSystem::updateimGUI_(float dt) {
  ...
  //          
		...
		for (auto& easingComponent : ECS.getAllComponents<Easing>())
		{
			const char* items[] = {
				"Linear",
				"EaseInOutSine",
			...
			static const char* current_item = items[easingComponent.actualEasing];

			auto& ent = ECS.entities[easingComponent.owner];
			string name = ("Easing -> " + ent.name);
			if (ImGui::TreeNode(name.c_str())) {

				if (ImGui::BeginCombo("Easing Function", current_item)) // The second parameter is the label previewed before opening the combo.
				{
					for (int n = 0; n < IM_ARRAYSIZE(items); n++)
					{
						bool is_selected = (current_item == items[n]); // You can store your selection however you want, outside or inside your objects
						if (ImGui::Selectable(items[n], is_selected)) {
							easingComponent.actualEasing = EasingMethod(n);
				...
				float duration = easingComponent.duration;
				ImGui::SliderFloat("Easing Duration", &duration, 0.1f, 10);
				easingComponent.duration = duration;

				bool easePos = easingComponent.ease_pos;
				ImGui::Checkbox("Tween Position", &easePos);
				easingComponent.ease_pos = easePos;
				if (easePos == true) {

					float from_pos[3] = { easingComponent.from_pos.x,easingComponent.from_pos.y,easingComponent.from_pos.z };


					ImGui::DragFloat3("From Position", from_pos);
					easingComponent.from_pos = lm::vec3(from_pos[0], from_pos[1], from_pos[2]);
					float to_pos[3] = { easingComponent.to_pos.x,easingComponent.to_pos.y,easingComponent.to_pos.z };

					ImGui::DragFloat3("To Position", to_pos);
					easingComponent.to_pos = lm::vec3(to_pos[0], to_pos[1], to_pos[2]);
				}
				bool easeRot = easingComponent.ease_rot;
				ImGui::Checkbox("Tween Rotation", &easeRot);
				easingComponent.ease_rot = easeRot;
				if (easeRot == true) {

					float from_rot[3] = { easingComponent.from_rot.x,easingComponent.from_rot.y,easingComponent.from_rot.z };


					ImGui::DragFloat3("From Rotation", from_rot);
					easingComponent.from_rot = lm::vec3(from_rot[0], from_rot[1], from_rot[2]);
					float to_rot[3] = { easingComponent.to_rot.x,easingComponent.to_rot.y,easingComponent.to_rot.z };

					ImGui::DragFloat3("To Rotation", to_rot);
					easingComponent.to_rot = lm::vec3(to_rot[0], to_rot[1], to_rot[2]);
				}

				bool easeScale = easingComponent.ease_scale;
				ImGui::Checkbox("Tween Scale", &easeScale);
				easingComponent.ease_scale = easeScale;
				if (easeScale == true) {

					float from_scale[3] = { easingComponent.from_scale.x,easingComponent.from_scale.y,easingComponent.from_scale.z };


					ImGui::DragFloat3("From Scale", from_scale);
					easingComponent.from_scale = lm::vec3(from_scale[0], from_scale[1], from_scale[2]);
					float to_scale[3] = { easingComponent.to_scale.x,easingComponent.to_scale.y,easingComponent.to_scale.z };

					ImGui::DragFloat3("To Scale", to_scale);
					easingComponent.to_scale = lm::vec3(to_scale[0], to_scale[1], to_scale[2]);
				}
