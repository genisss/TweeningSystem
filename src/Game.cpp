...
void Game::init(int w, int h) {
	...
	tweening_system_.Init();


	/******** SHADERS **********/

	graphics_system_.screen_background_color = lm::vec4(0.05f, 0.05f, 0.05f, 1.0f);
	...
	int ball_entity = ECS.createEntity("Teapot");
	...
	Easing& easing = ECS.createComponentForEntity<Easing>(ball_entity);
	easing.actualEasing = EaseInOutCirc;
	easing.to_pos = lm::vec3(5, 0, 0);
	easing.from_pos = lm::vec3(5, 5, 0);	
	easing.from_rot = lm::vec3(0, 0, -1);
	easing.to_rot = lm::vec3(-1, 0, 0);	
	easing.from_scale = lm::vec3(1, 1, 1);
	easing.to_scale = lm::vec3(2, 2, 2);
	easing.duration = 3;
	easing.time_progress = 0;
	easing.ease_rot = true;
	easing.ease_pos = true;
	easing.ease_scale = true;

	int cube_entity = ECS.createEntity("Cube");
	Mesh& cube_mesh = ECS.createComponentForEntity<Mesh>(cube_entity);
	cube_mesh.geometry = graphics_system_.createGeometryFromFile("data/assets/cube.obj");
	cube_mesh.material = graphics_system_.createMaterial();
	graphics_system_.getMaterial(cube_mesh.material).shader_id = phong_shader->program;
	Easing& easing2 = ECS.createComponentForEntity<Easing>(cube_entity);
	easing2.actualEasing = EaseInOutElastic;
	easing2.from_pos = lm::vec3(-5, 5, 0);
	easing2.to_pos = lm::vec3(-5, 0, 0);
	easing2.from_rot = lm::vec3(0, 1, 0);
	easing2.to_rot = lm::vec3(-1, 0, 0);
	easing2.from_scale = lm::vec3(2, 2, 2);
	easing2.to_scale = lm::vec3(1, 1, 1);
	easing2.duration = 5;
	easing2.time_progress = 0;
	easing2.ease_rot = true;
	easing2.ease_pos = true;
	easing2.ease_scale = true;
	...
	ECS.getComponentFromEntity<Transform>(plane_entity).translate(0, -1.0f, 0);*/
	...
	int light_ent2 = ECS.createEntity("Light2");
	Light& light2 = ECS.createComponentForEntity<Light>(light_ent2);
	light2.direction = lm::vec3(1.0f, -1.0f, 1.0f);
	light2.type = LightTypeDirectional;
